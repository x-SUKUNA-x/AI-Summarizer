...
export const PromptInputBox = React.forwardRef((props, ref) => {
  const {
    ...
    className,
    // External mic control
    onMicClick,
    isExternalRecording = false,
    // External audio file handler (overrides image-only)
    onFileSelect,
  } = props;
  ...
  const [internalRecording, setInternalRecording] = React.useState(false);
  ...
  // Use external recording state if onMicClick is provided
  const isRecording = onMicClick ? isExternalRecording : internalRecording;
  ...
  // Border goes red when either external or internal recording is active
    ...
    <>
      <PromptInput
        ...
        className={cn(
          ...
          isRecordingActive && "border-red-500/70",
          className
        )}
        ...
        <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
          <div
            ...
            <PromptInputAction tooltip={onFileSelect ? 'Upload audio file' : 'Upload image'}>
              <button
                ...
                <input
                  ...
                  accept={onFileSelect ? 'audio/*,.mp3,.wav,.m4a,.ogg,.webm' : 'image/*'} />
              </button>
            </PromptInputAction>
          ...
          <PromptInputAction
            ...
            <Button
              variant="default"
              size="icon"
              className={cn("h-8 w-8 rounded-full transition-all duration-200", isRecordingActive
              ...
              onClick={() => {
                if (onMicClick) {
                  if (isExternalRecording || hasContent) {
                    if (isExternalRecording) { onMicClick(); }
                    else { handleSubmit(); }
                  } else {
                    onMicClick();
                  }
                } else {
                  if (internalRecording) setInternalRecording(false);
                  else if (hasContent) handleSubmit();
                  else setInternalRecording(true);
                }
              }}
              disabled={isLoading && !hasContent && !isExternalRecording}>
              {isLoading ? (
